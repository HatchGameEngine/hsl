...
#define THROW_TOO_FEW_FMT_ERR(thread) \
	thread->ThrowRuntimeError("Too few arguments for format string!")
...
ObjString* npf_getarg_str(void* ctx) {
	...
	if (index < pc_cnt->argcount) {
		result = pc_cnt->thread->GetVMString(pc_cnt->args, index);
	}
	else {
...
int npf_getarg_int(void* ctx) {
	...
	int result = 0;
	if (index < pc_cnt->argcount) {
		result = pc_cnt->thread->GetInteger(pc_cnt->args, index);
	}
	else {
		THROW_TOO_FEW_FMT_ERR(pc_cnt->thread);
	}
...
float npf_getarg_float(void* ctx) {
	...
	float result = 0.0f;
	if (index < pc_cnt->argcount) {
		result = pc_cnt->thread->GetDecimal(pc_cnt->args, index);
	}
	else {
		THROW_TOO_FEW_FMT_ERR(pc_cnt->thread);
	}
...
ObjString* npf_vpprintf(char const* format, int argCount, VMValue* args, VMThread* thread) {
	...
	ObjString* result = thread->Manager->CopyString(textBuffer, pc_cnt.buffer.WriteIndex);
